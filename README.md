...
- SMTP, MQTT, and TFTP evidence surfacing when TShark exposes those fields, including decoded SMTP AUTH credential evidence and TFTP transfer grouping/export.
...
- Consolidated artifact manager output with `artifacts/manifest.json`; rejected artifacts are grouped in default stdout, with individual offsets preserved in JSON or verbose output. `files` and `suspicious` remain compatibility aliases with deprecation warnings.
